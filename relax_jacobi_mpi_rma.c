...
/**
 * This is a jacobi_mpi_rma 
 */
struct relaxation_jacobi_mpi_rma_hidden_params_s {
...
const struct relaxation_function_class_s _relaxation_jacobi_mpi_rma;

static struct relaxation_params_s*
jacobi_mpi_rma_relaxation_init(struct hw_params_s* hw_params){

    struct relaxation_jacobi_mpi_rma_hidden_params_s* rp;
    uint32_t np = hw_params->resolution;

    rp = (struct relaxation_jacobi_mpi_rma_hidden_params_s*)
            malloc(sizeof(struct relaxation_jacobi_mpi_rma_hidden_params_s));
    ...
    rp->super.rel_class = &_relaxation_jacobi_mpi_rma;

    // Initiate RMA 
    // double *mem;
    int memsize = rp->super.sizey*rp->super.sizex;
    // MPI_Win win;
    rp->mem[0] = (double*) calloc(1,memsize*sizeof(double));
    rp->mem[1] = (double*) calloc(1,memsize*sizeof(double));

    MPI_Win_create(rp->mem[0], memsize*sizeof(double), sizeof(double),
        MPI_INFO_NULL, MPI_COMM_WORLD, &rp->win[0]);
    MPI_Win_create(rp->mem[1], memsize*sizeof(double), sizeof(double),
        MPI_INFO_NULL, MPI_COMM_WORLD, &rp->win[1]);
 ...
 fail_and_return:
    ...
    MPI_Win_free(&rp->win[0]);
    MPI_Win_free(&rp->win[1]);
    MPI_Free_mem(rp->mem[0]);
    MPI_Free_mem(rp->mem[1]);
    free(rp);
    return NULL;
}

static int jacobi_mpi_rma_relaxation_fini(relaxation_params_t** prp){

    struct relaxation_jacobi_mpi_rma_hidden_params_s* rp = (struct relaxation_jacobi_mpi_rma_hidden_params_s*)*prp;
...
static int jacobi_mpi_rma_relaxation_coarsen(relaxation_params_t* grp, double* dst, uint32_t dstx, uint32_t dsty){
    struct relaxation_jacobi_mpi_rma_hidden_params_s* rp = (struct relaxation_jacobi_mpi_rma_hidden_params_s*)grp;

    return coarsen(rp->data[(rp->idx + 1) % 2], rp->super.sizex, rp->super.sizey,
                   dst, dstx, dsty);
}

static int jacobi_mpi_rma_relaxation_print(relaxation_params_t* grp, FILE* fp){
    struct relaxation_jacobi_mpi_rma_hidden_params_s* rp = (struct relaxation_jacobi_mpi_rma_hidden_params_s*)grp;
    fprintf( fp, "\n# Iteration %d\n", rp->idx);
...
/**
 * One step of a simple jacobi_mpi_rma relaxation.
 */
static double jacobi_mpi_rma_relaxation_apply(relaxation_params_t* grp){
    struct relaxation_jacobi_mpi_rma_hidden_params_s* rp = (struct relaxation_jacobi_mpi_rma_hidden_params_s*)grp;
    ...
    // Communication exchange between neighbors (rma) within submatrix
    int tag = 0;

    MPI_Win_fence(0, rp->win[(rp->idx + 1) % 2]);
    // north (send to north and receive from north)
    if (north != MPI_PROC_NULL){
        MPI_Put(&o[1*bx+1], 1, rp->north_south_type, north, (by-1)*bx+1 , 1, rp->north_south_type, rp->win[(rp->idx + 1) % 2]);   
    }
    // south
    if (south != MPI_PROC_NULL){
        MPI_Put(&o[(by-2)*bx+1], 1, rp->north_south_type, south, 0*bx+1, 1, rp->north_south_type, rp->win[(rp->idx + 1) % 2]);
    }
    // east
    if (east != MPI_PROC_NULL){
        MPI_Put(&o[1*bx+(bx-2)], 1, rp->east_west_type, east, 1*bx+0, 1, rp->east_west_type, rp->win[(rp->idx + 1) % 2]);
    }
    // west
    if (west != MPI_PROC_NULL){
        MPI_Put(&o[1*bx+1], 1, rp->east_west_type, west, 1*bx+(bx-1) , 1, rp->east_west_type, rp->win[(rp->idx + 1) % 2]);
    }
    MPI_Win_fence(0,rp->win[(rp->idx + 1) % 2]);
...
static double* jacobi_mpi_rma_relaxation_get_data(relaxation_params_t* grp){   
    struct relaxation_jacobi_mpi_rma_hidden_params_s* rp = (struct relaxation_jacobi_mpi_rma_hidden_params_s*)grp;
...
const struct relaxation_function_class_s _relaxation_jacobi_mpi_rma =
    {
        .type = RELAXATION_JACOBI_MPI_RMA,
		.method_name = "MPI RMA for Jacobi relaxation",
		._init = jacobi_mpi_rma_relaxation_init,
        ._fini = jacobi_mpi_rma_relaxation_fini,
        ._coarsen = jacobi_mpi_rma_relaxation_coarsen,
        ._print = jacobi_mpi_rma_relaxation_print,
        ._relaxation = jacobi_mpi_rma_relaxation_apply,
        ._get_data = jacobi_mpi_rma_relaxation_get_data,
    };
